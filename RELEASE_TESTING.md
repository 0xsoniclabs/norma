...
| `release_testing/features/genesis/`        | **Rotate.** The exception to the row above: the artifact has no feature to be the oldest supporter of, so the interesting pin is the release the candidate is expected to exchange g-files with — the newest one. Keep the node count at two, one per version. |
