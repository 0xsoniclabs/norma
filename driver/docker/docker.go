...
func (c *Client) CreateBridgeNetwork() (*Network, error) {
	...
	// create new network
	resp, err := c.cli.NetworkCreate(context.Background(), name, docker_network.CreateOptions{
		Labels: map[string]string{
			objectsLabel: "true",
		},
	})
...
func (c *Client) listNetworks() ([]docker_network.Summary, error) {
	return c.cli.NetworkList(context.Background(), docker_network.ListOptions{
