...
func TestRegisterValidatorNode_Failure(t *testing.T) {
	mockBackendForCreateValidator(t, func(backend *MockContractBackend) {
		...
		if err := RegisterValidatorNode(backend, ValidatorId(2)); err == nil || !strings.Contains(err.Error(), "failed to get receipt") {
			t.Fatalf("unexpected error; %v", err)
		}
	})
}
...
func TestRegisterValidatorNode_Failure_TransactionReverted(t *testing.T) {
	mockBackendForCreateValidator(t, func(backend *MockContractBackend) {
		...
		if err := RegisterValidatorNode(backend, ValidatorId(3)); err == nil || strings.Contains(err.Error(), "failed to get receipt") {
