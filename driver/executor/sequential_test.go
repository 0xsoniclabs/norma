...
func TestSequential_StartAndStopNode(t *testing.T) {
	...
	gomock.InOrder(
		...
		node.EXPECT().Stop().Return(nil),
		node.EXPECT().Cleanup().Return(nil),
	)
...
func TestSequential_RejoinNode(t *testing.T) {
	...
	gomock.InOrder(
		...
		node1.EXPECT().Cleanup().Return(nil),
		// Rejoin: no registration (same name seen before)
...
func TestSequential_UpdateRulesWithEpochChange(t *testing.T) {
	ctrl := gomock.NewController(t)
	net := driver.NewMockNetwork(ctrl)
	net.EXPECT().GetActiveNodes().Return(nil)
	net.EXPECT().DialRandomRpc().Return(nil, fmt.Errorf("no nodes")).AnyTimes()
	...
	gomock.InOrder(
		net.EXPECT().ApplyNetworkRules(gomock.Any()).Return(nil),
	...
	scenario := parser.SequentialScenario{
		...
		Steps: []parser.Step{
			{
				Function:           parser.FuncUpdateRules,
				Rules:              map[string]string{"UPGRADES_BRIO": "true"},
				WaitForEpochChange: true,
			},
		},
	}
...
func TestSequential_Check(t *testing.T) {
	...
	checker.EXPECT().Check().Return(nil)
