...
// execution, and the scenario's Timeout is enforced as a deadline.
func RunSequential(
	ctx context.Context,
...
func executeStep(
	...
	case parser.FuncStopNode:
		return execStopNode(step, net, state)
	...
	case parser.FuncCheckBlocksProduced:
		return execCheck("blocks_rolling", step, checks)
	case parser.FuncCheckBlocksHalted:
		return execCheck("blocks_halted", step, checks)
	case parser.FuncCheckBlockHashes:
		return execCheck("blocks_hashes", step, checks)
	case parser.FuncCheckBlockHeights:
		return execCheck("block_height", step, checks)
	case parser.FuncCheckBlockGasRate:
		return execCheck("block_gas_rate", step, checks)
...
func execStartNode(
	...
	targetBlock, _ := getNetworkBlockHeight(ctx, net)
...
func execUpdateRules(step *parser.Step, net driver.Network) error {
	...
	if step.WaitForEpochChange {
		if err := net.AdvanceEpoch(1); err != nil {
			return fmt.Errorf("failed to advance epoch after rule update: %w", err)
		}
	}
...
func execCheck(checkerName string, step *parser.Step, checks checking.Checks) error {
	if checks == nil {
