...
// allStepFunctions is the set of all known step function names.
var allStepFunctions = map[string]StepFunction{
	"startNode":           FuncStartNode,
	"stopNode":            FuncStopNode,
	"undelegate":          FuncUndelegate,
	"updateRules":         FuncUpdateRules,
	"advanceEpoch":        FuncAdvanceEpoch,
	"waitForEpoch":        FuncWaitForEpoch,
	"runApp":              FuncRunApp,
	"stopApp":             FuncStopApp,
	"checkBlocksProduced": FuncCheckBlocksProduced,
	"checkBlocksHalted":   FuncCheckBlocksHalted,
	"checkBlockHashes":    FuncCheckBlockHashes,
	"checkBlockHeights":   FuncCheckBlockHeights,
	"checkBlockGasRate":   FuncCheckBlockGasRate,
	"waitFor":             FuncWaitFor,
}
...
func toStepFunction(s string) (StepFunction, error) {
	if fn, ok := allStepFunctions[s]; ok {
		return fn, nil
	}
...
func (s *SequentialScenario) setDefaults() {
	...
	if s.InitialRules["MAX_EPOCH_DURATION"] == "" {
		s.InitialRules["MAX_EPOCH_DURATION"] = "15s"
	}
}
