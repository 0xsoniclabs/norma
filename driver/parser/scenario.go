...
var checkParamDescriptions = map[string]string{
	...
	"tolerance":      "For a height check, the allowed deviation (int, in blocks) between nodes. For a production or halt check, the length of the observation window expressed in monitoring samples (one per second); duration overrides it.",
