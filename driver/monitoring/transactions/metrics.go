...
// The ceiling the contributions to a block had to fit into. It is the network's
// MaxBlockGas rule, a technical hard limit that a network is not normally
// expected to reach - what a block actually carries is governed by the gas the
// validators may spend per second and by the gas an event may carry.
var BlockGasLimit = mon.Metric[mon.Network, mon.Series[mon.BlockNumber, int]]{
	Name:        "BlockGasLimit",
...
func init() {
	...
	if err := mon.RegisterSource(BlockGasLimit, func(monitor *mon.Monitor) mon.Source[mon.Network, mon.Series[mon.BlockNumber, int]] {
		return newGasLimitSource(BlockGasLimit, monitor)
	}); err != nil {
		panic(fmt.Sprintf("failed to register metric source: %v", err))
	}
