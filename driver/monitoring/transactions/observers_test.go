...
func TestBlockGasLimit_ReadsTheLimitFromTheHeader(t *testing.T) {
	...
	limits, err := blockLimits(client, 42, newRulesCache())
	require.NoError(err)
	require.Equal(uint64(5_000_000_000), limits.GasLimit)
	require.Equal(uint64(2_500_000), limits.EventGasLimit,
		"the event limit is a rule of the network, read for the block it applied to")
...
func TestBlockGasLimit_ReportsAnUnknownBlock(t *testing.T) {
	...
	_, err := blockLimits(client, 42, newRulesCache())
	require.ErrorContains(t, err, "not found")
}
...
func TestInclusionObserver_ReadsEveryBlockOnce(t *testing.T) {
	...
	require.Equal(
		[]BlockLimit{{
			...
			EventGasLimit:  2_500_000,
			GasPowerPerSec: 30_000_000,
		}},
		tracker.BlockGasLimits(),
		"the limits of the block are recorded along with its transactions",
	)
}
...
func TestReadBlock_RetriesANodeThatIsBehind(t *testing.T) {
	...
	contents, err := readBlock(t.Context(), client, 12, newRulesCache())
	require.NoError(err)
	require.Len(contents.transactions, 1)
	require.Equal(uint64(5_000_000_000), contents.limits.GasLimit)
...
func TestReadBlock_GivesUpOnABlockThatStaysUnknown(t *testing.T) {
	...
	_, err := readBlock(t.Context(), client, 12, newRulesCache())
	require.ErrorContains(t, err, "not found")
}
...
type fakeClient struct {
	*rpc.MockClient
	...
	rules     func(epoch string) (opera.Rules, error)
	seen      map[string][]any
}
...
func newFakeClient(t *testing.T, responses map[string]string) *fakeClient {
	...
	// The rules are read for every block, for the limits its gas competed for.
	defaults := opera.Rules{}
	defaults.Economy.Gas.MaxEventGas = 2_500_000
	defaults.Economy.ShortGasPower.AllocPerSec = 30_000_000
	client.MockClient.EXPECT().GetNetworkRules(gomock.Any()).AnyTimes().
		DoAndReturn(func(epoch string) (opera.Rules, error) {
			if client.rules != nil {
				return client.rules(epoch)
			}
			return defaults, nil
		})
	return client
...
func TestRulesCache_ReadsTheRulesOncePerEpoch(t *testing.T) {
	require := require.New(t)

	reads := 0
	client := newFakeClient(t, nil)
	client.rules = func(epoch string) (opera.Rules, error) {
		reads++
		rules := opera.Rules{}
		rules.Economy.Gas.MaxEventGas = 2_500_000
		return rules, nil
	}

	cache := newRulesCache()
	for range 5 {
		rules, err := cache.rulesOf(client, 3)
		require.NoError(err)
		require.Equal(uint64(2_500_000), rules.Economy.Gas.MaxEventGas)
	}
	require.Equal(1, reads, "the rules of an epoch do not change, so one read is enough")

	_, err := cache.rulesOf(client, 4)
	require.NoError(err)
	require.Equal(2, reads, "a new epoch is read")
}

func TestRulesCache_RejectsRulesTheClientDoesNotKnow(t *testing.T) {
	// eth_getRules answers a null for an epoch it does not know, which decodes
	// into rules of all zeros without an error. Recording those would report a
	// network whose events may carry no gas at all.
	client := newFakeClient(t, nil)
	client.rules = func(string) (opera.Rules, error) { return opera.Rules{}, nil }

	_, err := newRulesCache().rulesOf(client, 7)
