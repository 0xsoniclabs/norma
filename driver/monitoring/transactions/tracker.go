...
// BlockLimit is the gas limit one block was formed under, which is the ceiling
// the contributions to it had to fit into.
...
type Tracker struct {
	...
	// limits holds the gas limit of each observed block, the ceiling those
	// contributions competed for.
...
// MarkBlockGasLimit reports the gas limit the given block was formed under. It
// is recorded for every observed block, including the ones that carried no
// transaction, so that the limit can be read next to what the blocks used.
func (t *Tracker) MarkBlockGasLimit(height int, gasLimit uint64) {
	t.mu.Lock()
	defer t.mu.Unlock()
	t.limits[height] = gasLimit
}
...
// BlockGasLimits returns the gas limit of every observed block, ordered by
// block height.
func (t *Tracker) BlockGasLimits() []BlockLimit {
	...
	for block, gasLimit := range t.limits {
		limits = append(limits, BlockLimit{Block: block, GasLimit: gasLimit})
	}
