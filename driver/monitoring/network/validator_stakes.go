...
// ValidatorStake tracks validator self-stakes obtained from the SFC contract.
var ValidatorStake = mon.Metric[mon.Node, mon.Series[mon.Time, string]]{
	Name:        "ValidatorStake",
	Description: "The self stake per validator obtained from SFC.GetStake().",
}
...
func NewValidatorStakeSource(monitor *mon.Monitor) mon.Source[mon.Node, mon.Series[mon.Time, string]] {
	return newValidatorStakeSourceWithCollector(monitor, time.Second, func() (map[int]string, error) {
...
func fetchValidatorStakes(network driver.Network) (map[int]string, error) {
	...
	for validatorID := 1; validatorID <= int(lastValidatorID.Int64()); validatorID++ {
		...
		delegator := crypto.PubkeyToAddress(evmcore.FakeKey(uint32(validatorID)).PublicKey)

		stake, err := sfcContract.GetStake(nil, delegator, validatorIDBig)
		if err != nil {
			return nil, fmt.Errorf("failed to query stake for validator %d: %w", validatorID, err)
		}
