...
func TestTrafficGenerating(t *testing.T) {
	...
	net, err := local.NewLocalLegacyNetwork(t.Context(),
	...
	generators, err := app.NewGenerators("erc20", 0, 0)
	if err != nil {
		t.Fatal(err)
	}
	...
	app, err := controller.NewAppController(generators, constantShaper, numGenerators, true, appContext, net)
	if err != nil {
		t.Fatal(err)
	}
	...
	time.Sleep(3 * time.Second) // wait for txs in TxPool
