...
// NewOsakaCounterApplication generates a P-256 key pair, deploys an OsakaCounter
...
func NewOsakaCounterApplication(ctxt AppContext, feederId, appId uint32) (Application, error) {
	...
	_, receipt, err := DeployContract(ctxt, func(opts *bind.TransactOpts, backend bind.ContractBackend) (common.Address, *types.Transaction, *contract.OsakaCounter, error) {
		return contract.DeployOsakaCounter(opts, backend, pubKeyX, pubKeyY)
	})
	if err != nil {
...
// OsakaCounterApplication represents a deployed OsakaCounter contract.
...
func (f *OsakaCounterApplication) GetReceivedTransactions(rpcClient rpc.Client) (uint64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("failed to get OsakaCounter contract; %w", err)
	}
...
// OsakaCounterUser sends incrementCounter transactions to the OsakaCounter contract.
